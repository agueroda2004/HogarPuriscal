import Menu from "./Menu.svg";
import Close from "./CloseBtn.svg";
import FacebookIcon from "./Facebook.svg";
import WhatsAppIcon from "./WhatsApp.svg";
import Calendar from "./Calendar.svg";
import ArrowDown from "./ArrowDown.svg";
export const assets = {
  Menu,
  Close,
  FacebookIcon,
  WhatsAppIcon,
  Calendar,
  ArrowDown,
};

export const imgUrl = {
  Logo: "https://ik.imagekit.io/5zi86k8wt/Hogar%20Puriscal/LogoHogar.png?updatedAt=1754013565804",
  HeroImagen:
    "https://ik.imagekit.io/5zi86k8wt/Hogar%20Puriscal/ImagenCorousel.png?updatedAt=1754019311315",
  QuienesSomos:
    "https://ik.imagekit.io/5zi86k8wt/Hogar%20Puriscal/QuienesSomos.png?updatedAt=1754225651580",
  TrabajoComunal:
    "https://ik.imagekit.io/5zi86k8wt/Hogar%20Puriscal/TrabajoComunal.png?updatedAt=1754225655932",
  Voluntariado:
    "https://ik.imagekit.io/5zi86k8wt/Hogar%20Puriscal/Voluntariado.png?updatedAt=1754225659345",
};

export const menuLinks = [
  {
    name: "Inicio",
    path: "/",
  },
  {
    name: "Servicios",
    path: "/working",
  },
  {
    name: "Donaciones",
    path: "/working",
  },
  {
    name: "Requisitos",
    path: "/working",
  },
  {
    name: "Colaboradores",
    path: "/working",
  },
  {
    name: "Quienes somos",
    path: "/quienes-somos",
  },
  {
    name: "Voluntariado",
    path: "/working",
  },
  {
    name: "Trabajo comunal",
    path: "/working",
  },
  {
    name: "Preguntas frecuentes",
    path: "/working",
  },
  {
    name: "Contacto",
    path: "/contacto",
  },
];

export const InfoCards = [
  {
    title: "Servicios",
    description:
      "Cuidado integral 24 horas con asistencia médica, nutrición y actividades para el bienestar de residentes.",
    image:
      "https://ik.imagekit.io/5zi86k8wt/Hogar%20Puriscal/Servicios.png?updatedAt=1754225659600",
    alt: "Servicios",
  },
  {
    title: "Requisitos",
    description:
      "Proceso de admisión sencillo que incluye información médica y contactos de emergencia para una transición segura.",
    image:
      "https://ik.imagekit.io/5zi86k8wt/Hogar%20Puriscal/Requisitos.png?updatedAt=1754225644557",
    alt: "Requisitos",
  },
  {
    title: "Donaciones",
    description:
      "Agradecemos donaciones en especie, dinero o tiempo voluntario para mantener la calidad de atención.",
    image:
      "https://ik.imagekit.io/5zi86k8wt/Hogar%20Puriscal/Donaciones.png?updatedAt=1754225659889",
    alt: "Donaciones",
  },
  {
    title: "Colaboradores",
    description:
      "Invitamos a voluntarios y profesionales a unirse para crear un ambiente cálido y de bienestar.",
    image:
      "https://ik.imagekit.io/5zi86k8wt/Hogar%20Puriscal/Colaboradores.png?updatedAt=1754225653315",
    alt: "Colaboradores",
  },
  {
    title: "Quiénes somos",
    description:
      "Equipo comprometido en ofrecer un entorno seguro, cálido y respetuoso para adultos mayores.",
    image:
      "https://ik.imagekit.io/5zi86k8wt/Hogar%20Puriscal/QuienesSomos.png?updatedAt=1754330437164",
    alt: "Quiénes somos",
  },
  {
    title: "Voluntariado",
    description:
      "Oportunidad para aportar tiempo y cariño, transformando la vida de nuestros residentes.",
    image:
      "https://ik.imagekit.io/5zi86k8wt/Hogar%20Puriscal/Voluntariado.png?updatedAt=1754225659345",
    alt: "Voluntariado",
  },
  {
    title: "Trabajo comunal",
    description:
      "Espacios para que estudiantes aprendan y aporten con empatía en el cuidado de adultos mayores.",
    image:
      "https://ik.imagekit.io/5zi86k8wt/Hogar%20Puriscal/TrabajoComunal.png?updatedAt=1754225655932",
    alt: "Trabajo comunal",
  },
  {
    title: "Preguntas frecuentes",
    description:
      "Respuestas a dudas comunes sobre servicios, admisión, visitas y colaboración.",
    image:
      "https://ik.imagekit.io/5zi86k8wt/Hogar%20Puriscal/PreguntasFrecuentes.png?updatedAt=1754225639566",
    alt: "Preguntas frecuentes",
  },
  {
    title: "Contacto",
    description:
      "Información y atención para consultas sobre servicios y procesos.",
    image:
      "https://ik.imagekit.io/5zi86k8wt/Hogar%20Puriscal/Contacto.png?updatedAt=1754225659440",
    alt: "Contacto",
  },
];

export const quienesSomos = [
  {
    title: "Acerca del hogar",
    description: [
      "En el Hogar Corazón de Jesús procuramos el bienestar físico, emocional y social de nuestros residentes.",
      "Actualmente, además de los cuidados básicos, les ofrecemos diariamente acompañamiento nutricional, fisioterapia, atención de enfermería y nuestro doctor en Geriatría, el más reciente logro en función del bienestar de los adultos mayores.",
      "Las Religiosas Franciscanas de la Purísima nos colaboraron hasta enero de 2017 con sus atenciones y su soporte espiritual. Actualmente se cuenta con una coordinadora general y una junta administrativa que, junto con el resto de empleados y voluntarios, mantienen el funcionamiento del hogar.",
      "El grupo de voluntariado es un apoyo fundamental para la organización de actividades recreativas, además del tiempo que comparten con cada uno de los adultos mayores.",
      "Este hogar es una institución sin fines de lucro y se financia con recursos provenientes de la J.P.S., FODESAF, CONAPAM y el IMA, además de donaciones de la comunidad y los ingresos generados por los festejos populares de cada año.",
      "Sin embargo, los fondos no son suficientes para desarrollar todos los proyectos que podrían aplicarse en este hogar, ya que las salidas diarias por necesidades básicas son realmente significativas. Por ello, apelamos a una mayor colaboración de la comunidad —comercios y personas individuales— que deseen apoyarnos, ya sea con dinero en efectivo o ayudas en especie: productos de la canasta básica, verduras, frutas, vegetales, productos de limpieza, ropa, paños, etc.",
      "También es necesario un mayor involucramiento del voluntariado, ya sea integrándose al grupo de Damas Voluntarias o brindando su ayuda individual o en grupo para una actividad o necesidad específica.",
    ],
  },
  {
    title: "Visión",
    description: [
      "Ofrecer a cada uno de los residentes del hogar una atención integral dentro del marco legal actual, promoviendo a la vez el trabajo en equipo con la participación de los residentes, sus familias, representantes o encargados.",
    ],
  },
  {
    title: "Misión",
    description: [
      "Mantener una calidad de vida digna para los adultos mayores por medio de aspectos sociales, físicos, emocionales y espirituales, que nos permita madurar el ideal de una expectativa de vida alta e integral.",
    ],
  },
  {
    title: "Objetivo",
    description: [
      "Contribuir por todos los medios lícitos a mejorar el sistema de vida de las personas adultas mayores más necesitadas del cantón y sus alrededores, brindándoles un mejor lugar donde vivir con calidad de vida y donde puedan pasar dignamente esa maravillosa etapa de la vida.",
    ],
  },
  {
    title: "Filosofía",
    description: [
      "Respetar la integridad del residente dentro del marco legal.",
    ],
  },
  {
    title: "Política institucional",
    description: [
      "Conjunto de prioridades y estrategias que guían a la organización, junto con la comunidad y sus familias, para satisfacer las necesidades físicas, mentales, biológicas, espirituales, sociales y laborales productivas de la persona adulta mayor, con el objetivo de buscar una mejor calidad de vida para estas personas.",
    ],
  },
];

export const SeccionDonaciones = {
  title: "¿Desea ayudarnos?",
  description: `En nuestro hogar de adultos mayores trabajamos cada día para brindar atención, cuidado y un ambiente lleno de respeto y cariño a quienes más lo necesitan. Puede apoyarnos a través de donaciones económicas, insumos o dedicando su tiempo como voluntario. Cada aporte, por pequeño que parezca, marca una gran diferencia en la vida de nuestros adultos mayores.`,
  image:
    "https://ik.imagekit.io/5zi86k8wt/Hogar%20Puriscal/Seccion1.png?updatedAt=1755295619898",
  links: [
    {
      title: "Trabajo comunal",
      path: "/trabajo-comunal",
    },
    {
      title: "Voluntariado",
      path: "/voluntariado",
    },
    {
      title: "Donaciones",
      path: "/donaciones",
    },
  ],
  color: "DABE57",
};

export const SeccionHogar = {
  title: "Hogar Puriscal",
  description: `En nuestro hogar de ancianos, A Su Lado, creemos firmemente que cada residente merece un ambiente cálido, seguro y lleno de cariño. Aquí, más que un lugar para vivir, encontrará una verdadera familia dedicada a su bienestar, ofreciendo atención personalizada y actividades que enriquecen cada día.`,
  image:
    "https://ik.imagekit.io/5zi86k8wt/Hogar%20Puriscal/SeccionHogar.jpg?updatedAt=1755312777739",
  links: [
    {
      title: "Quienes somos",
      path: "/quienes-somos",
    },
    {
      title: "Colaboradores",
      path: "/colaboradores",
    },
    {
      title: "Servicios",
      path: "/servicios",
    },
  ],
  color: "CABC92",
};

export const SeccionPreguntas = {
  title: "¿Tiene preguntas?",
  description: `Si desea conocer más sobre nuestros servicios, programas o formas de apoyo, estamos aquí para ayudarle. Nuestro equipo estará encantado de atender sus consultas y brindarle toda la información que necesite para sentirse seguro y confiado en su decisión. Contáctenos por teléfono, correo electrónico o a través de nuestro formulario en línea.`,
  image:
    "https://ik.imagekit.io/5zi86k8wt/Hogar%20Puriscal/Seccion2.png?updatedAt=1755296118188",
  links: [
    {
      title: "Preguntas",
      path: "/preguntas-frecuentes",
    },
    {
      title: "Requisitos",
      path: "/requisitos",
    },
    {
      title: "Contacto",
      path: "/contacto",
    },
  ],
  color: "AE2C17",
};
